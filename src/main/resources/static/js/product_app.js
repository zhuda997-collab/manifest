...
function handleSearch() {
    ...
    if (!kw) {
        ...
        currentData = allData.filter(item =>
            ...
            String(item.productNo).includes(kw) ||
            (item.submodelName && item.submodelName.toLowerCase().includes(kw))
        );
    }
    currentPage = 1;
    renderTable();
...
function renderTable() {
    ...
    tbody.innerHTML = pageItems.map(item => {
        ...
        return `<tr>
            ...
            <td>${esc(item.submodelName || '—')}</td>
            <td style="text-align:right;font-weight:600;">¥${priceYuan}</td>
...
function openModal(mode, id) {
    ...
    if (mode === 'edit' && id) {
        ...
        document.getElementById('productNo').value = item.productNo;
        document.getElementById('productName').value = item.productName;
        document.getElementById('submodelNo').value = item.submodelNo ?? '';
        document.getElementById('submodelName').value = item.submodelName || '';
        document.getElementById('unitPriceYuan').value = (item.unitPrice / 100).toFixed(2);
        document.getElementById('productNo').readOnly = true;
    } else {
...
async function submitForm(e) {
    ...
    const unitPriceYuan = parseFloat(document.getElementById('unitPriceYuan').value) || 0;
